...
- **horizon_weeks**: 6
- **main_objective**: PRD와 AGENTS.md를 단일 기준으로 삼아 OCR, 번역, 인페인팅, 식자 파이프라인을 6주 내
  MVP로 정렬하고 페이지당 평균 30초 처리와 읽기 품질 게이트를 충족하는 구현 로드맵을 제공한다.
- **hard_gate_definition**: Hard Gate는 input_text_count와 output_text_count 일치,
  bbox_overflow 0, min_font_pt 10 이상을 동시에 만족해야 한다.
- **tdd_execution_principle**: 모든 기능은 RED, GREEN, REFACTOR 순서로 진행하며 테스트가 명세를
  대표하고 구현은 해당 테스트를 통과하는 최소 범위로 시작한다.
...
## Phase 1: Google 로그인/결제 승인 포함 인증, 업로드, 작업 생성 API 기초

- **목표 기간**: W1
- **예상 소요 시간**: 4h
- **상태**: pending
- **목표**: Google OAuth 로그인과 FastAPI 기반 signup/login/refresh/logout, 토스 결제 승인
  prepare/confirm API, ZIP 업로드, 작업 생성 엔드포인트를 TDD로 고정하고 Idempotency-Key 규칙을 도입한다.
- **완료 신호(게이트)**: Google 로그인, 결제 승인, 기존 로그인 모두에서 작업 생성까지 API 호출이 정상 동작하고 입력 오류에서
  app_code가 일관되게 반환된다.

### Phase 1 - 산출물/수용 기준

- Auth signup/login/refresh/logout와 POST /auth/google API 골격 완성
- POST /auth/google 요청은 {"id_token":"string"}을 받고 성공 시 access_token,
  refresh_token, user(id,email,name,avatar_url)를 반환한다.
- POST /payments/prepare 요청은
  {"orderId":"string","amount":number,"orderName":"string","customerKey":"string"}을
  저장하고 expiresAt을 반환한다.
- POST /payments/confirm 요청은
  {"paymentKey":"string","orderId":"string","amount":number}으로 토스 승인 API를 호출하고
  승인 성공 시 크레딧 원장에 충전 기록을 남긴다.
...
### Phase 1 - 개발 원칙 체크리스트 (RED/GREEN/REFACTOR)

- [ ] **RED**: test_auth_google_exchanges_valid_id_token,
  test_auth_google_rejects_invalid_token,
  test_auth_google_requires_client_id_env, test_payments_prepare_stores_order,
  test_payments_confirm_rejects_amount_mismatch,
  test_create_job_requires_idempotency_key를 실패 상태로 고정한다.
- [ ] **GREEN**: POST /auth/google, POST /payments/prepare, POST
  /payments/confirm, POST /jobs, ZIP 파싱, app_code 매핑을 최소 구현으로 통과시킨다.
- [ ] **REFACTOR**: Google 토큰 검증 어댑터, 결제 승인 서비스, Auth 서비스 경계를 분리하고 예외 처리 코드를
  통합한다.
...
- [ ] RED 테스트를 구현 코드보다 먼저 작성했다.
- [ ] RED 실패 원인을 테스트 명세와 연결해 기록했다.
- [ ] GREEN 이후 REFACTOR에서 테스트를 변경하지 않았다.

#### Phase 1 - 빌드/테스트

- [ ] 백엔드 관련 테스트 명령이 모두 통과한다.
- [ ] 검증 명령 validate와 check가 통과한다.

#### Phase 1 - 코드 품질

- [ ] Pydantic 스키마와 타입 선언이 누락되지 않았다.
- [ ] 에러 응답은 HTTP 상태 코드와 app_code를 함께 반환하며 Google 인증 app_code를 포함한다.

#### Phase 1 - 보안/성능

- [ ] 민감 정보는 코드에 하드코딩하지 않고 환경 변수로 분리했다.
- [ ] Google ID 토큰을 서버에서 검증하고 profile.getId()를 백엔드 식별자로 사용하지 않는다.
- [ ] 결제 시크릿 키를 서버에서만 사용하고 Basic 인증 인코딩 값을 클라이언트에 노출하지 않는다.
- [ ] 결제 승인 10분 제한과 amount/orderId 무결성 검증으로 위변조를 차단한다.

#### Phase 1 - 문서화

- [ ] API 입력 오류와 app_code 매핑을 문서에 반영했다.
- [ ] Phase 결과와 남은 이슈를 진행률 추적에 업데이트했다.

#### Phase 1 - 수동 검증

- [ ] Google 로그인 성공 후 ZIP 업로드와 작업 생성 수동 경로를 점검했다.
- [ ] Google 로그아웃(signOut) 이후 보호 경로 재접근 차단을 확인했다.
- [ ] 결제 승인 성공 후 크레딧 충전과 credit_ledger 기록 생성을 수동 검증했다.

### Phase 1 - 품질 게이트 검증 명령(예시)
...
- google-signin-client_id는 구현 시 NEXT_PUBLIC_GOOGLE_SIGNIN_CLIENT_ID 환경 변수로 주입하는
  것을 기본값으로 한다.
- 백엔드 스키마는 GoogleAuthRequest, GoogleAuthResponse, GoogleUserPayload,
  PaymentPrepareRequest, PaymentPrepareResponse, PaymentConfirmRequest,
  PaymentConfirmResponse, TossApproveResponse를 기준으로 정의한다.
- 결제 환경 변수 키 표준은 TOSS_WIDGET_CLIENT_KEY, TOSS_SECRET_KEY,
  NEXT_PUBLIC_TOSS_WIDGET_CLIENT_KEY를 사용한다.

## Phase 2: 큐, 워커, 상태 모델 SSOT 고정

- **목표 기간**: W2
- **예상 소요 시간**: 4h
- **상태**: pending
- **목표**: Redis 큐와 워커를 연결하고 job_status 및 page_status 전이를 PRD SSOT와 일치시키며 취소와 재시도
  규칙을 구현한다.
- **완료 신호(게이트)**: 작업 취소, 페이지 재시도, 상태 전이가 SSOT에 맞게 동작하고
  PAGE_RETRY_LIMIT_EXCEEDED가 일관되게 반환된다.
...
### Phase 2 - 개발 원칙 체크리스트 (RED/GREEN/REFACTOR)

- [ ] **RED**: test_active_job_limit_per_user, test_page_retry_limit_three,
  test_cancel_job_transitions_to_canceled를 먼저 실패시킨다.
- [ ] **GREEN**: 큐 enqueue와 dequeue, 취소 및 재시도 API를 최소 구현으로 통과시킨다.
- [ ] **REFACTOR**: 상태 전이 로직을 서비스 계층으로 모으고 중복 분기를 제거한다.
...
- [ ] 재시도와 취소 규칙의 RED 테스트를 먼저 작성했다.
- [ ] SSOT 전이 케이스를 테스트 명세에 반영했다.

#### Phase 2 - 빌드/테스트

- [ ] 큐, 워커, 상태 전이 관련 테스트가 통과한다.
- [ ] 문서 동기화 check가 통과한다.

#### Phase 2 - 코드 품질

- [ ] 상태 전이 분기가 단일 서비스 함수로 관리된다.
- [ ] 예외 코드는 PRD app_code 목록과 충돌이 없다.

#### Phase 2 - 보안/성능

- [ ] 활성 작업 1건 제한을 서버에서 강제한다.
- [ ] 큐 적체 시 차단 코드 반환으로 과부하를 제어한다.

#### Phase 2 - 문서화

- [ ] 상태 모델 표를 문서와 구현에서 동일하게 유지한다.
- [ ] 재시도 제한 규칙을 API 설명에 반영한다.

#### Phase 2 - 수동 검증

- [ ] 작업 취소 후 페이지 상태가 canceled로 고정되는지 점검한다.
- [ ] 재시도 상한 초과 시 UI 메시지가 일관적인지 확인한다.

### Phase 2 - 품질 게이트 검증 명령(예시)
...
## Phase 3: OCR, 번역, 폴백, Hard Gate 결합

- **목표 기간**: W3
- **예상 소요 시간**: 4h
- **상태**: pending
- **목표**: OpenAI를 기본 경로로 사용하고 장애 시 Gemini Vision 폴백을 적용하며 Hard Gate 실패 시
  fail-closed로 종료한다.
- **완료 신호(게이트)**: 텍스트 수 보존과 레이아웃 하한 조건을 자동 검증하고 외부 장애 시 폴백 후에도 실패 정책을 유지한다.
...
### Phase 3 - 개발 원칙 체크리스트 (RED/GREEN/REFACTOR)

- [ ] **RED**: test_hard_gate_fails_on_text_loss,
  test_fallback_runs_when_openai_timeout, test_fail_closed_when_fx_rate_missing를
  실패 상태로 만든다.
- [ ] **GREEN**: OCR와 번역 파이프라인, Hard Gate 검증, 폴백 분기 최소 구현을 통과시킨다.
- [ ] **REFACTOR**: 외부 API 어댑터를 분리하고 타임아웃 및 에러 매핑 중복을 제거한다.
...
- [ ] Hard Gate 실패 케이스를 RED로 먼저 고정했다.
- [ ] 폴백 분기 성공과 실패 시나리오를 모두 테스트에 반영했다.

#### Phase 3 - 빌드/테스트

- [ ] 워커 파이프라인 테스트와 비용 제한 테스트가 통과한다.
- [ ] 문서 검증 명령이 통과한다.

#### Phase 3 - 코드 품질

- [ ] OpenAI와 Gemini 호출 어댑터 경계를 명확히 분리했다.
- [ ] 게이트 조건식이 테스트 명세와 동일하다.

#### Phase 3 - 보안/성능

- [ ] 일일 비용 상한 초과 시 신규 작업 차단이 강제된다.
- [ ] 실패 정책은 fail-closed를 유지한다.

#### Phase 3 - 문서화

- [ ] Hard Gate 기준과 폴백 정책을 문서에 반영했다.
- [ ] 비용 및 환율 예외 정책을 운영 섹션에 정리했다.

#### Phase 3 - 수동 검증

- [ ] 외부 API 장애를 가정한 수동 폴백 점검을 수행한다.
- [ ] 게이트 실패 시 사용자 노출 메시지를 검토한다.

### Phase 3 - 품질 게이트 검증 명령(예시)
...
## Phase 4: 인페인팅, 식자, 페이지 편집 및 재시도 고도화

- **목표 기간**: W4
- **예상 소요 시간**: 4h
- **상태**: pending
- **목표**: 레이아웃 품질을 보장하는 식자 규칙과 페이지 편집 가능 조건을 확정하고 실패 페이지 재시도 흐름을 안정화한다.
- **완료 신호(게이트)**: bbox_overflow와 min_font_pt 조건을 만족하고 편집 제한 규칙이 서버와 UI에서 동일하게
  동작한다.
...
### Phase 4 - 개발 원칙 체크리스트 (RED/GREEN/REFACTOR)

- [ ] **RED**: test_layout_gate_rejects_overflow,
  test_layout_gate_rejects_too_small_font,
  test_page_edit_blocked_when_not_editable를 실패로 고정한다.
- [ ] **GREEN**: 식자 규칙과 편집 제한 분기를 최소 구현으로 통과시킨다.
- [ ] **REFACTOR**: 식자 파라미터와 페이지 상태 검사 코드를 분리해 유지보수성을 높인다.
...
- [ ] 레이아웃 실패와 편집 제한 케이스를 RED로 먼저 작성했다.
- [ ] 게이트 조건 경계값 테스트를 포함했다.

#### Phase 4 - 빌드/테스트

- [ ] 식자 게이트와 페이지 편집 테스트가 통과한다.
- [ ] 문서 동기화 check가 통과한다.

#### Phase 4 - 코드 품질

- [ ] 레이아웃 계산 로직의 책임 경계를 분리했다.
- [ ] 오류 코드와 메시지 매핑을 중앙화했다.

#### Phase 4 - 보안/성능

- [ ] 페이지 재시도 상한 3회 규칙을 서버에서 강제한다.
- [ ] 과도한 편집 요청에 대한 제한 정책을 검토한다.

#### Phase 4 - 문서화

- [ ] 레이아웃 게이트 기준과 편집 제한 규칙을 문서화한다.
- [ ] 페이지 상태별 재시도 정책을 운영 가이드에 반영한다.

#### Phase 4 - 수동 검증

- [ ] 완료된 페이지와 실패 페이지 편집 동작을 수동 점검한다.
- [ ] 다양한 해상도 샘플에서 말풍선 넘침 여부를 확인한다.

### Phase 4 - 품질 게이트 검증 명령(예시)
...
## Phase 5: Next.js 사용자 여정, Google 로그인 UI, 결제위젯, 오류 매핑 통합

- **목표 기간**: W5
- **예상 소요 시간**: 4h
- **상태**: pending
- **목표**: Google 로그인 UI, 토스 결제위젯 UI, 업로드, 진행률 조회, 결과 다운로드, 페이지 편집, 신고 생성까지 프론트
  사용자 여정을 통합하고 app_code 기반 오류 표시를 표준화한다.
- **완료 신호(게이트)**: 핵심 사용자 여정에서 업로드부터 다운로드까지 단절 없이 동작하고 오류 메시지가 app_code 기준으로 일관되게
  노출된다.

### Phase 5 - 산출물/수용 기준

- 로그인 페이지에 <script src="https://apis.google.com/js/platform.js" async
  defer></script>를 반영한다.
- 로그인 페이지에 <meta name="google-signin-client_id"
  content="455519487200-7liplou3k773k5sc5539fpk4fedk6am2.apps.googleusercontent.com">를
  반영한다.
- g-signin2 버튼과 onSignIn 콜백에서 프로필 표시와 ID 토큰 교환 흐름을 연결한다.
- signOut 함수에서 gapi.auth2.getAuthInstance().signOut() 호출을 반영한다.
- 결제 페이지에 <script src="https://js.tosspayments.com/v2/standard"></script> 또는
  @tosspayments/tosspayments-sdk 초기화를 반영한다.
...
- successUrl과 failUrl 리다이렉트 파라미터(paymentKey, orderId, amount, code, message)를
  처리한다.
...
### Phase 5 - 개발 원칙 체크리스트 (RED/GREEN/REFACTOR)

- [ ] **RED**: google-login.spec, payment-widget.spec,
  payment-success-page.spec, payment-fail-page.spec, upload.spec,
  job-detail.spec, error-map.spec를 먼저 작성하고 실패 상태를 확인한다.
- [ ] **GREEN**: Google 로그인 UI, 결제위젯 UI, 업로드, 상세, 오류 안내 화면을 최소 구현으로 연결한다.
- [ ] **REFACTOR**: React Query 중심으로 서버 상태를 분리하고 Google/결제위젯 타입 경계와 컴포넌트 책임을
  정리한다.
...
- [ ] UI 단위 테스트를 기능 구현보다 먼저 작성했다.
- [ ] 오류 매핑 케이스를 RED에 포함했다.

#### Phase 5 - 빌드/테스트

- [ ] Frontend 단위 테스트 명령이 통과한다.
- [ ] 문서 validate와 check가 통과한다.

#### Phase 5 - 코드 품질

- [ ] 컴포넌트는 함수형 패턴과 Hook 규칙을 준수한다.
- [ ] window.gapi, GoogleUser, BasicProfile, AuthResponse 타입 선언에서 any를 사용하지 않는다.
- [ ] TossPayments/widgets 관련 타입 선언에서 any를 사용하지 않는다.

#### Phase 5 - 보안/성능

- [ ] 클라이언트에 민감 정보가 노출되지 않도록 점검하고 클라이언트 ID를 환경 변수로 주입한다.
- [ ] 결제 시크릿 키는 프론트로 전달하지 않고 결제위젯 클라이언트 키만 노출한다.
- [ ] 장시간 폴링 또는 과도한 재요청을 방지한다.

#### Phase 5 - 문서화

- [ ] 오류 코드별 UI 대응표를 문서로 유지한다.
- [ ] 사용자 여정 스크린 단위 설명을 업데이트한다.

#### Phase 5 - 수동 검증

- [ ] Google 로그인부터 결제위젯 진입, 업로드-다운로드까지 브라우저 수동 경로를 점검한다.
- [ ] 결제 성공 리다이렉트(paymentKey/orderId/amount) 처리와 실패 리다이렉트(code/message) 안내 문구를
  확인한다.
- [ ] Google 로그아웃 후 세션 해제와 대표 오류 코드 안내 문구를 확인한다.

### Phase 5 - 품질 게이트 검증 명령(예시)
...
- window.gapi, GoogleUser, BasicProfile, AuthResponse 글로벌 타입 선언을 프론트 인증 모듈 기준으로
  정의한다.
- TossPaymentsInstance, TossPaymentsWidgets, RequestPaymentParams,
  PaymentRedirectSuccessQuery, PaymentRedirectFailQuery 타입 선언을 결제 모듈 기준으로 정의한다.

## Phase 6: E2E, KPI, Go 또는 No-Go 판정

- **목표 기간**: W6
- **예상 소요 시간**: 4h
- **상태**: pending
- **목표**: Playwright 기반 사용자 여정 E2E와 KPI 집계를 연결해 Hard Gate, Soft Gate, 사용자 Gate를
  최종 점검하고 MVP 출시 여부를 판단한다.
- **완료 신호(게이트)**: E2E 사용자 여정이 통과하고 Hard Gate, Soft Gate, 사용자 Gate 지표가 PRD 기준을
  충족한다.
...
### Phase 6 - 개발 원칙 체크리스트 (RED/GREEN/REFACTOR)

- [ ] **RED**: user-journey.e2e, payment-credit-topup.e2e, metrics.test,
  moderation.test를 먼저 작성하고 실패를 확인한다.
- [ ] **GREEN**: E2E 자동화, 결제 성공/실패 리다이렉트 경로, KPI 집계 코드를 최소 구현으로 통과시킨다.
- [ ] **REFACTOR**: 플레이키 완화 전략, 결제 테스트 픽스처 분리, 지표 계산 모듈 분리를 반영한다.
...
- [ ] E2E와 KPI 테스트를 구현 전에 RED로 작성했다.
- [ ] 게이트 기준을 테스트와 문서에서 동일하게 유지한다.

#### Phase 6 - 빌드/테스트

- [ ] E2E와 통합 KPI 테스트 명령이 통과한다.
- [ ] 문서 동기화와 검증 명령이 통과한다.
- [ ] 결제 관련 E2E(payment-credit-topup) 테스트가 통과한다.

#### Phase 6 - 코드 품질

- [ ] KPI 계산식은 PRD 정의와 동일한 수식을 사용한다.
- [ ] 테스트 데이터 생성 코드의 중복을 줄인다.

#### Phase 6 - 보안/성능

- [ ] 콘텐츠 정책 위반 신고 흐름을 E2E에서 점검한다.
- [ ] 페이지당 평균 처리 시간과 타임아웃 지표를 함께 검토한다.

#### Phase 6 - 문서화

- [ ] Go 또는 No-Go 판정 근거를 문서로 남긴다.
- [ ] 학습 사항과 차기 개선 과제를 기록한다.

#### Phase 6 - 수동 검증

- [ ] 샘플 계정으로 결제 승인 후 크레딧 반영과 작업 생성 가능 상태를 수동 검증한다.
- [ ] 관리자 신고 처리 흐름을 수동으로 점검한다.

### Phase 6 - 품질 게이트 검증 명령(예시)
...
- **결정**: 로드맵 문서의 단일 원본은 Plans.json으로 고정한다.
- **근거**: 기계 편집 안정성을 확보하고 Plans.md를 항상 결정론적으로 생성하기 위함이다.
- **결정**: 문서 성격은 즉시 실행 체크리스트가 아닌 6주 미래 구현 로드맵으로 유지한다.
- **근거**: 현재 저장소 단계에서는 실행 지시보다 단계 목표와 의존성, 게이트 관리가 우선이다.
- **결정**: 테스트 명령 정책은 example_only를 유지한다.
- **근거**: 구현 이전 단계에서 파일 경로가 변동될 수 있어 명령은 예시로 관리하는 편이 현실적이다.
- **결정**: Hard Gate 실패는 fail-closed를 기본 정책으로 채택한다.
- **근거**: 품질 미달 결과를 사용자에게 노출하지 않기 위해 보수적 실패 정책이 필요하다.
- **결정**: 외부 AI 경로는 OpenAI 기본, Gemini Vision 폴백으로 고정한다.
- **근거**: AGENTS.md 스택 강제 사항과 PRD 내결함성 요구를 동시에 만족한다.
- **결정**: MVP Google 로그인 경로는 platform.js + g-signin2를 사용한다.
- **근거**: 사용자 요구사항의 스니펫을 정확히 반영하고 초기 통합 속도를 확보하기 위함이다.
- **결정**: 백엔드는 Google ID 토큰 검증 후 자체 JWT를 발급한다.
- **근거**: 세션 관리 일관성과 보안 정책을 유지하기 위함이다.
- **결정**: 토스페이먼츠 결제위젯은 v2를 MVP 기준으로 고정한다.
- **근거**: 사용자 제공 가이드를 기준으로 빠르게 결제 통합을 완료하기 위함이다.
- **결정**: 결제 API 계약은 POST /payments/prepare + POST /payments/confirm 분리 구조로
  고정한다.
- **근거**: 결제 요청과 승인 사이 무결성 검증을 명확히 하고 리다이렉트 흐름을 안정화하기 위함이다.
- **결정**: 결제 승인 성공 시 비즈니스 반영은 크레딧 충전으로 고정한다.
...
- **트리거**: Google 인증 실패율 급증, 결제 승인 실패율 급증, 또는 인증/업로드 API에서 잘못된 app_code가 반복적으로
  반환된다.
- **복구 목표**: Phase 1 시작 전 API 계약 상태
- **롤백 단계**:
...
- **트리거**: 상태 전이 불일치 또는 큐 적체로 작업 취소가 불가능해진다.
- **복구 목표**: Phase 1 완료 상태
- **롤백 단계**:
...
- **트리거**: 폴백 경로가 오작동해 Hard Gate 실패 데이터가 노출된다.
- **복구 목표**: Phase 2 완료 상태
- **롤백 단계**:
...
- **트리거**: 식자 품질 게이트가 무력화되어 레이아웃 오류가 증가한다.
- **복구 목표**: Phase 3 완료 상태
- **롤백 단계**:
...
- **트리거**: 프론트 화면에서 오류 코드 안내가 잘못 표시된다.
- **복구 목표**: Phase 4 완료 상태
- **롤백 단계**:
...
- **트리거**: KPI 집계 오류 또는 E2E 불안정으로 Go 판정이 불가능하다.
- **복구 목표**: Phase 5 완료 상태
- **롤백 단계**:
...
- **구현 코드 미존재**: 로드맵 문서를 기준으로 Phase 1부터 TDD 순서로 실제 코드와 테스트를 생성한다.
